...
namespace CoreDapperRepository.Core.Data
{
    public interface IRepository<T> where T : BaseEntity
    {
        ...
        /// <summary>
        /// 删除数据（默认以主键删除，这样设计是因为可能不同数据库的主键定义不一样(int类型或者guid类型的主键)，这里定义默认主键格式：Id，类型为：int）
        /// </summary>
        ...
        /// <param name="useTransaction">是否开启事务</param>
        /// <returns>执行结果（true or false）</returns>
        bool Delete(int entityId, string predicate = "", object param = null, int? commandTimeout = null, bool useTransaction = false);
